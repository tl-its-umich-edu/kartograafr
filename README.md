...
Using Docker for the run time environment has a couple of design
implications.
...
to install Anaconda and use it to create a virtual environment
including Python 3 and the esri package distribution.  The Dockerfile
shows how to do that in a Docker container.  For a local development
server download the appropriate distribution from
https://www.anaconda.com/download

Pip can be installed in Anaconda virtual environments so that the
familiar Python package management can be done when required.

Once anaconda is installed and Python 3 virtual environment is created
and activated
install the ArcGIS api by running  **conda install -c esri arcgis**
After that development can proceed as usual.  Note that you must use
the python provided by the virtual environment.  See Anaconda
installation information from the Anaconda site.
...
log configuration in **config.py** must point to existing directories.
...
on OpenShift since that provides a rich enviroment of services. 
...
-------------------- below here is old text to be deleted.


## need to explain the cron-env script.

## add https://devumich.maps.arcgis.com/home/index.html

## describe need for persistant logs
